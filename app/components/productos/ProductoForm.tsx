...
export default function ProductoForm({ 
  ...
  // Establecer moneda por defecto (EUR) si hay monedas disponibles
  useEffect(() => {
    if (monedas.length > 0 && !form.moneda_id) {
      const monedaPorDefecto = monedas.find(m => m.codigo === 'EUR') || monedas[0];
      setForm({ ...form, moneda_id: monedaPorDefecto.id });
    }
  }, [monedas]);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-col items-center gap-3 border-b pb-6 mb-2">
            ...
            <p className="text-xs text-gray-500">
              {uploading ? 'Procesando imagen...' : 'Click para subir imagen del producto (máx. 2MB)'}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onKeyDown={preventSpin}
              />
              <p className="text-xs text-gray-500 mt-1">Peso en kilogramos</p>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Largo × Ancho × Alto en centímetros</p>
          </div>
